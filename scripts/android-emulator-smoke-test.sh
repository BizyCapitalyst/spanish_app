...
fail_with_log() {
  ...
  if [ -f "$LOGFILE" ]; then
    ...
    adb logcat -d -v threadtime | grep -Ei -C 15 "FATAL EXCEPTION|AndroidRuntime|Process: ${APP_ID}|${APP_ID}" | tail -300 || true
  fi
  exit 1
}
...
grep -Fq "Status: ok" "$STARTFILE" || fail_with_log "cold launch did not report Status: ok"
sleep "$WAIT_SECONDS"
...
adb logcat -c
adb shell am start -W -n "$APP_ID/$ACTIVITY" | tee "$STARTFILE"
grep -Fq "Status: ok" "$STARTFILE" || fail_with_log "second launch did not report Status: ok"
sleep "$WAIT_SECONDS"
